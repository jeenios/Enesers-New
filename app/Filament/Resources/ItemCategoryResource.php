...
class ItemCategoryResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Section::make('Item Categories Information')
                    ->schema([
                        Grid::make(1)->schema([
                            TextInput::make('code')
                                ->label('Code')
                ...
                // Section::make('Items in this Category')
                //     ->schema([
                //         HasManyRepeater::make('item') // pakai relasi item()
                //             ->schema([
                //                 TextInput::make('code')
                //                     ->label('Item Code')
                //                     ->required(),
                ...
                //                 TextInput::make('general_description')
                //                     ->label('Description'),
                //             ])
                //             ->columns(3)
                //             ->collapsible()
                        ...
                        ->before(function ($record, \Filament\Tables\Actions\DeleteAction $action) {
                            if ($record->item()->exists()) {
                                Notification::make()
                                    ...
                                    ->body('Item Category ini masih digunakan oleh item, silakan hapus item terlebih dahulu.')
                                    ->danger()
                                    ->send();
                ...
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make()
                        ->before(function ($records, DeleteBulkAction $action) {
                            $usedCategories = $records->filter(fn($record) => $record->item()->exists());
