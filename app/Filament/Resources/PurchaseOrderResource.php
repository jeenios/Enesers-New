...
class PurchaseOrderResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Tabs::make('Purchase Order Information')
                    ->tabs([
                            ...
                            ->schema([
                                Grid::make(1)
                                    ->schema([
                                        ...
                                        Repeater::make('purchaseOrderMultiple')
                                            ->relationship('purchaseOrderMultiple')
                                            ->columns(4)
                                            ->collapsible()
                                ...
                                Select::make('purchase_requisition_id')
                                    ...
                                    ->afterStateUpdated(function ($state, callable $set) {
                                        if ($state) {
                                            ...
                                            $set('purchaseOrderMultiple', $prItems->map(fn($i) => [
                                                'item_id'     => $i->item_id,
                                                'description' => $i->description,
                                                'quantity'    => $i->quantity,
                                                'unit_id'     => $i->unit_id,
    ...
    // {
    //     if (!empty($data['purchaseOrderMultiple'])) {
    //         foreach ($data['purchaseOrderMultiple'] as $poItem) {
    //             // Update existing jika ada id, atau buat baru jika tidak ada
    //             if (!empty($poItem['id'])) {
    //                 $record->purchaseOrderMultiple()->where('id', $poItem['id'])->update([
    //                     'item_id'         => $poItem['item_id'],
    //                     'description'     => $poItem['description'] ?? null,
    //                     'quantity'        => $poItem['quantity'] ?? 1,
    //                     'unit_id'         => $poItem['unit_id'],
    //                     'price'           => $poItem['price'] ?? 0,
    //                     'toggle_discount' => $poItem['toggle_discount'] ?? false,
    //                     'discount_id'     => $poItem['discount_id'] ?? null,
    //                     'tax_id'          => $poItem['tax_id'] ?? null,
    //                 ]);
    //             } else {
    //                 \App\Models\PurchaseOrderMultiple::create([
    //                     'purchase_order_id'          => $record->id,
    //                     'purchase_order_multiple_id' => $poItem['purchase_order_multiple_id'] ?? null,
    //                     'item_id'                    => $poItem['item_id'],
    //                     'description'                => $poItem['description'] ?? null,
    //                     'quantity'                   => $poItem['quantity'] ?? 1,
    //                     'unit_id'                    => $poItem['unit_id'],
    ...
    protected static function syncItemsFromPR($state, callable $set, callable $get): void
    {
        if ($state) {
            ...
            $existingPO = collect($get('purchaseOrderMultiple') ?? []);

            // Inject ke repeater
            $set('purchaseOrderMultiple', $prItems->map(function ($i) use ($existingPO) {
    ...
    public static function afterSave(\Illuminate\Database\Eloquent\Model $record, array $data): void
    {
        $poItems = $data['purchaseOrderMultiple'] ?? [];

        foreach ($poItems as $item) {
            PurchaseOrderMultiple::updateOrCreate(
                ['id' => $item['id'] ?? null],
                [
                    'purchase_order_id' => $record->id,
                    ...
                    'price'             => $item['price'] ?? 0,
                    'toggle_discount'   => $item['toggle_discount'] ?? false,
